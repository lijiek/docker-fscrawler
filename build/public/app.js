...
//      host: process.env.INTERFACE || localhost,
      baseUrl: 'http://10.242.29.192:3000', // API url
      searchTerm: 'python', // Default search term
  ...
  methods: {
    ...
    async nextBookPage () {
      ...
      this.paragraphs = await this.getParagraphs(this.selectedParagraph._source.meta.title, this.bookOffset + 10)
    },
    ...
    async showBookModal (searchHit) {
      try {
        ...
        this.paragraphs = await this.getParagraphs(searchHit._source.meta.title,  0 )     //searchHit.content
      } catch (err) {
        console.error(err)
      }
    },
